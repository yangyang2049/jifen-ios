...
enum WatchScoreActionProjector {
    ...
    static func basketball(
        ...
        let scores = (state.leftScore, state.rightScore)
        let period = state.currentPeriod
        switch intent {
        case .addPoints(let side, let points, _):
            var actions = [action(
                .scoreAdd, code: "basketball_score", side: side,
                scores: scores, delta: points, periodNumber: period, timestamp: timestamp
            )]
            if state.finished {
                actions.append(action(
                    .gameEnd, code: "game_end", scores: scores,
                    periodNumber: period, winner: state.leftScore == state.rightScore
                        ? nil : (state.leftScore > state.rightScore ? .left : .right),
                    timestamp: timestamp
                ))
            }
            return actions
        case .adjustScore(let side, let delta):
            return [action(
                .editScore, code: "basketball_adjust_score", side: side,
                scores: scores, delta: delta, periodNumber: period, timestamp: timestamp
            )]
        case .addFoul(let side):
            return [action(
                .foul, code: "basketball_foul", side: side,
                scores: scores, delta: 1, periodNumber: period, timestamp: timestamp
            )]
        case .removeFoul(let side):
            return [action(
                .foul, code: "basketball_foul_adjust", side: side,
                scores: scores, delta: -1, periodNumber: period, timestamp: timestamp
            )]
        case .useTimeout(let side):
            return [action(
                .timeout, code: "basketball_timeout", side: side,
                scores: scores, delta: 1, periodNumber: period, timestamp: timestamp
            )]
        case .adjustTimeout(let side, let delta):
            return [action(
                .timeout, code: "basketball_timeout_adjust", side: side,
                scores: scores, delta: delta, periodNumber: period, timestamp: timestamp
            )]
        case .advanceToNextPeriod:
            return [action(
                .periodEnd, code: "basketball_period_changed",
                scores: scores, periodNumber: period, timestamp: timestamp
            )]
        case .enterOvertime:
            return [action(
                .periodEnd, code: "basketball_overtime",
                scores: scores, periodNumber: period, timestamp: timestamp
            )]
        case .selectPeriod(let selected):
            return [action(
                .stateChange, code: "basketball_select_period",
                scores: scores, periodNumber: selected, timestamp: timestamp
            )]
        case .exchangeSides:
            return [action(
                .sideChange, code: "exchange_sides",
        ...
        case .setRuleSet:
            return [action(
                .stateChange, code: "basketball_rule_changed",
                scores: scores, periodNumber: period, timestamp: timestamp
            )]
        case .finish:
            return [action(
        ...
        case .reset, .setClockRunning, .tickClock, .resetGameClock, .resetShotClock:
            return []
        }
    }
