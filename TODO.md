...
- [ ] **多人计分「轮次模式」（iOS 侧）**〔主表 P0，2026-09-24 登记，本版本不动手〕：iOS 零实现——`MultiScoreboardView.swift:572,631` 只有一次性 `settleMatch`，全仓无 `multiRound`/`轮次`（UNO 的 `showUnoRoundPanel` 是另一套，不能当轮次模式复用）。鸿蒙 3.2 为契约来源：`jifen-hos/shared/src/main/ets/scoreboard/core/s3/multiParticipantTypes.ts`（`multiScoreMode: 'continuous' \| 'round_based'`、`MultiRoundState{ number, entries, touched, settled[], undo[], actions[] }`）、`multiRoundReducer.ts`（`set_round_score`/`settle_round`/`undo_round` + `expectedRevision`）、`persistence/multiDualTrackJsonCodec.ts`。落地要求：① 引擎与意图语义按上面对齐，缺 `multiScoreMode` 时按 `continuous` 解码；② 记录投影**必须同时写摊平字段** `multiScoreMode` / `multiScoreSnapshot`，每轮一条 `SET_END` 带 `roundNumber` 与 `multiScoreRoundEntries[{playerId, scoreDelta}]`，`totalRounds` = 含该字段的动作条数——只写 `stateSnapshot` blob 会让安卓读不出（正是「跨端续打 blob↔flattened」那条架构级缺口的同款坑），与 `ScoreboardRecord.swift:997` 现有扁平 extraData 键规范同批；③ 恢复按 `revision` 大者胜（移植 `preferMultiRoundCheckpoint` 判定顺序），并确认 `ScoreboardRecordPresentation.swift:244` 的 `isReliableForResume` 闸门对轮次记录不误放行；④ 详情页补轮次/阶段记录渲染出口（现在只有 `ScoreboardRecordDetailPage.swift:840` 的 `.periodFinished`「本节结束」），显示端 `DisplayStateWireCodec.swift` 同步加轮次区块；⑤ UI：开局选模式、本轮改分→结算→轮次历史，层级对齐鸿蒙 `MultiRoundSettlementDialog` / `MultiRoundHistoryDialog`；⑥ 文案三套（en / zh-Hans / zh-Hant）+ `LocalizationIntegrityTests` 键集合断言同步。验收：四人「逐轮结算」三轮后杀进程恢复轮次与累计分一致，同一条记录安卓与网页显示端读出的轮次增量完全相同。
